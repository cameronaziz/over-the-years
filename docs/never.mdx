...
for (const person of people) {
	...
	// We could have just used `.map()` and then `.filter()`.
	// But with a for loop, we can break out!
...
```
#### Solution
All the way back at the beginnig, we defined `firstFiveClassmates` as an empty array, but we never told Typescript compiler it will be *an array of what*. One part of the code could be adding a `Person` to the array, and another could be adding a `Llama` - because who doesn't want to have a llama in their class?
...
```typescript
const firstFiveClassmates: Array<Person | Llama> = [] // Because llamas are allowes
```
